//! A simple binary to test the simple environment variables template.

use askama::Template;
use std::collections::HashMap;

/// Template for the simple environment variables page.
#[derive(Template)]
#[template(path = "pages/simple_env.html")]
struct SimpleEnvTemplate {
    has_env: bool,
...
fn main() {
    ...
    env_vars.push(("DEBUG".to_string(), "true".to_string()));
    
    let template_with_env = SimpleEnvTemplate {
        has_env: true,
        env_vars,
    };
    ...
    let template_without_env = SimpleEnvTemplate {
        has_env: false,
        env_vars: Vec::new(),
    };
